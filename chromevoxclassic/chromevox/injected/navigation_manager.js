...
/**
 * New create method to construct the NavigationManager
 */
cvox.NavigationManager.create = async function() {
  navManager = new cvox.NavigationManager();
...
/**
 * initializing the NavigationManager
 */
cvox.NavigationManager.prototype.init = async function() {
  this.addInterframeListener_();
 ...
 */
cvox.NavigationManager.prototype.reset = async function() {
  /**
 ...
 */
cvox.NavigationManager.prototype.resolve = async function(opt_predicate) {
 ...
 * @private
 */
cvox.NavigationManager.prototype.hasNext_ = async function() {
  ...
  await emptyNavShifter.sync(emptySel);
  if (await emptyNavShifter.next(emptySel)) {
    result = true;
  }
  return result;
};
 ...
 */
cvox.NavigationManager.prototype.findNext = async function(
  ...
  this.shifterStack_ = [];
  var ret = await cvox.FindUtil.findNext(this.curSel_, predicate, opt_initialNode);
  if (!this.ignoreIframesNoMatterWhat_) {
    this.tryIframe_(ret && ret.start.node);
  }
  if (ret) {
    await this.updateSelToArbitraryNode(ret.start.node);
  }
  this.predicate_ = '';
  return ret;
};
 ...
 * Delegates to NavigationShifter with current page state.
 */
cvox.NavigationManager.prototype.sync = async function() {
  await this.resolve();
  var ret = await this.shifter_.sync(this.curSel_);
  if (ret) {
    this.curSel_ = ret;
  }
};
 ...
 */
cvox.NavigationManager.prototype.syncAll = async function(opt_skipText) {
  await this.sync();
...
cvox.NavigationManager.prototype.getDescription = async function() {
  ...
  // Selected content.
  var desc = this.pageSel_ ? await this.pageSel_.getDescription(
          this.shifter_, this.prevSel_, this.curSel_) :
      await this.shifter_.getDescription(this.prevSel_, this.curSel_);
  var earcons = [];
 ...
 */
cvox.NavigationManager.prototype.getBraille = async function() {
  return await this.shifter_.getBraille(this.prevSel_, this.curSel_);
};
 ...
 */
cvox.NavigationManager.prototype.performAction = async function(name) {
  var newSel = null;
  switch (name) {
    case 'enterShifter':
 ...
 * defaults to true.
 */
cvox.NavigationManager.prototype.makeMoreGranular = async function(opt_persist) {
 ...
 * defaults to true.
 */
cvox.NavigationManager.prototype.makeLessGranular = async function(opt_persist) {
 ...
 * Delegates to NavigationShifter.
 */
cvox.NavigationManager.prototype.ensureSubnavigating = async function() {
 ...
 */
cvox.NavigationManager.prototype.ensureNotSubnavigating = async function() {
  if (this.shifter_.isSubnavigating()) {
 ...
 */
cvox.NavigationManager.prototype.finishNavCommand = async function(
  ...
  var descriptionArray = await cvox.ChromeVox.navigationManager.getDescription();
 ...
 */
cvox.NavigationManager.prototype.navigate = async function(
  ...
  await this.ensureNotSubnavigating();
  ...
  return await this.next_(!opt_ignoreIframes);
};
 ...
 * call to navigate().
 */
cvox.NavigationManager.prototype.subnavigate = async function() {
  this.pageEndAnnounced_ = false;
  if (!(await this.resolve())) {
    return;
  }
  await this.ensureSubnavigating();
  await this.next_(true);
};
...
cvox.NavigationManager.prototype.startCallbackReading_ =
    cvox.ChromeVoxEventSuspender.withSuspendedEvents(async function(queueMode) {
  await this.finishNavCommand('', true, queueMode, goog.bind(async function() {
    if (this.prevReadingSel_ == this.curSel_) {
      this.stopReading();
      return;
    }
    this.prevReadingSel_ = this.curSel_;
    if (await this.next_(true) && this.keepReading_) {
...
cvox.NavigationManager.prototype.startNonCallbackReading_ =
  ...
  if (!cvox.ChromeVox.tts.isSpeaking()) {
    await this.finishNavCommand('', true, queueMode, null);
    if (!(await this.next_(true))) {
      this.keepReading_ = false;
    }
  }
 ...
 * @return {Array<cvox.NavDescription>} The summary of the current position.
 */
cvox.NavigationManager.prototype.getFullDescription = async function() {
  if (this.pageSel_) {
    return this.pageSel_.getFullDescription();
  }
  return [await cvox.DescriptionUtil.getDescriptionFromAncestors(
  ...
  cvox.Interframe.addListener(function(message) {
    ...
    cvox.ChromeVoxEventSuspender.withSuspendedEvents(async function() {
      ...
      if (message['findNext']) {
        ...
        var found = await self.findNext(predicate, predicateName, true);
      ...
      await self.finishNavCommand('', true);
 ...
 * walker).
 */
cvox.NavigationManager.prototype.updateSelToArbitraryNode = async function(
    node, opt_precise) {
  if (node) {
