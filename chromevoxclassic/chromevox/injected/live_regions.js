 ...
 */
cvox.LiveRegions.init = async function(pageLoadTime, queueMode, disableSpeak) {
  ...
  for (var i = 0; i < regions.length; i++) {
    await cvox.LiveRegions.handleOneChangedNode(
        regions[i],
        regions[i],
        false,
        false,
 ...
 */
cvox.LiveRegions.handleOneChangedNode = async function(
  ...
  await cvox.LiveRegions.announceChangeIfVisible(node, liveRoot, isRemoval, handler);
};
 ...
 *     Callback function to be called for each live region found.
 */
cvox.LiveRegions.announceChangeIfVisible = async function(
    node, liveRoot, isRemoval, handler) {
  if (cvox.DomUtil.isVisible(liveRoot)) {
    await cvox.LiveRegions.announceChange(node, liveRoot, isRemoval, handler);
  } else {
 ...
 *     Callback function to be called for each live region found.
 */
cvox.LiveRegions.announceChange = async function(
    node, liveRoot, isRemoval, handler) {
  ...
  var navDescriptions = await cvox.LiveRegions.getNavDescriptionsRecursive(node);
  if (isRemoval) {
    navDescriptions = [await cvox.DescriptionUtil.getDescriptionFromAncestors(
 ...
 *     at this node.
 */
cvox.LiveRegions.getNavDescriptionsRecursive = async function(node) {
  ...
  return await cvox.DescriptionUtil.getFullDescriptionsFromChildren(null,
      /** @type {!Element} */ (node));
};
