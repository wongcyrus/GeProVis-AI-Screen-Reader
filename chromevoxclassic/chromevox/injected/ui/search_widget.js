 ...
 * @override
 */
cvox.SearchWidget.prototype.hide = async function(opt_noSync) {
  if (this.isActive()) {
  ...
  cvox.ChromeVox.navigationManager.speakDescriptionArray(
      await cvox.ChromeVox.navigationManager.getDescription(),
 ...
 * @override
 */
cvox.SearchWidget.prototype.onKeyDown = async function(evt) {
  if (!this.isActive()) {
    return false;
  }
  ...
  if (evt.keyCode == 8) { // Backspace
    if (searchStr.length > 0) {
      ...
      await this.beginSearch_(searchStr);
    } else {
      await cvox.ChromeVox.navigationManager.updateSelToArbitraryNode(
          this.initialNode);
      await cvox.ChromeVox.navigationManager.syncAll();
    }
  } else if (evt.keyCode == 40) { // Down arrow
    await this.next_(searchStr, false);
  } else if (evt.keyCode == 38) { // Up arrow
    await this.next_(searchStr, true);
 ...
 * @override
 */
cvox.SearchWidget.prototype.onKeyPress = async function(evt) {
  if (!this.isActive()) {
    return false;
  }
 ...
 */
cvox.SearchWidget.prototype.nextResult = async function(opt_reverse) {
  if (!this.isActive()) {
    return null;
  }
  var searchStr = this.txtNode_.value;
  return await this.next_(searchStr, opt_reverse);
};
 ...
 * @private
 */
cvox.SearchWidget.prototype.getNextResult_ = async function(searchStr) {
  ...
  do {
    ...
    var descriptions = await cvox.ChromeVox.navigationManager.getDescription();
  ...
  } while (await cvox.ChromeVox.navigationManager.navigate(true,
 ...
 * @private
 */
cvox.SearchWidget.prototype.beginSearch_ = async function(searchStr) {
  var result = await this.getNextResult_(searchStr);
 ...
 * @private
 */
cvox.SearchWidget.prototype.next_ = async function(searchStr, opt_reversed) {
  cvox.ChromeVox.navigationManager.setReversed(!!opt_reversed);

  var success = false;
  if (this.getPredicate()) {
    success = await cvox.ChromeVox.navigationManager.findNext(
    ...
    success = await cvox.ChromeVox.navigationManager.navigate(true);
  }
  var result = success ? await this.getNextResult_(searchStr) : null;
  this.outputSearchResult_(result, searchStr);
  this.onNavigate();
  return result;
};
 ...
 * @private
 */
cvox.SearchWidget.prototype.textFromCurrentDescription_ = async function() {
  var descriptions = await cvox.ChromeVox.navigationManager.getDescription();
  var text = '';
  for (var i = 0; i < descriptions.length; i++) {
 ...
 * @private
 */
cvox.SearchWidget.prototype.handleSearchChanged_ = async function(evt) {
  var searchStr = evt.target.value + evt.data;
  await cvox.SearchWidget.prototype.beginSearch_(searchStr);
};
