 ...
 */
cvox.PdfProcessor.finish = async function() {
 ...
 */
cvox.PdfProcessor.onMessage = async function(message) {
  ...
  if (cvox.PdfProcessor.pageCount !== null) {
    await cvox.PdfProcessor.processOnePage(info);
    return;
  }
  ...
  await cvox.PdfProcessor.getNextPage();
};
 ...
 */
cvox.PdfProcessor.getNextPage = async function() {
  ...
  if (cvox.PdfProcessor.pageIndex >= cvox.PdfProcessor.pageCount) {
    ...
    cvox.PdfProcessor.pdfEmbed.parentElement.appendChild(
        cvox.PdfProcessor.documentDiv);
    await cvox.PdfProcessor.finish();
    return;
  }
 ...
 */
cvox.PdfProcessor.processOnePage = async function(info) {
