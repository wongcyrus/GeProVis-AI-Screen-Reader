 ...
 */
cvox.ChromeVoxEventWatcher.init = async function(doc) {
  /**
   * @type {Object}
   */
...
cvox.ChromeVoxEventWatcher.secondPassThroughKeyUp_ = false;
  ...
  await cvox.ChromeVoxEventWatcher.addEventListeners_(doc);
};
 ...
 * @private
 */
cvox.ChromeVoxEventWatcher.addEventListeners_ = async function(doc) {
  ...
  cvox.ChromeVoxEventWatcher.addEventListener_(doc,
      'focus', await cvox.ChromeVoxEventWatcher.focusEventWatcher, true);
  cvox.ChromeVoxEventWatcher.addEventListener_(doc,
 ...
 * @private
 */
cvox.ChromeVoxEventWatcher.setLastFocusedNode_ = async function(element) {
  cvox.ChromeVoxEventWatcher.lastFocusedNode = element;
  cvox.ChromeVoxEventWatcher.lastFocusedNodeValue = !element ? null :
      await cvox.DomUtil.getControlValueAndStateString(element);
};
 ...
 * @return {boolean} True if the default action should be performed.
 */
cvox.ChromeVoxEventWatcher.focusEventWatcher = async function(evt) {
  ...
  if (!cvox.ChromeVoxEventSuspender.areEventsSuspended()) {
    ...
    await cvox.ChromeVoxEventWatcher.setLastFocusedNode_(
 ...
 */
cvox.ChromeVoxEventWatcher.focusHandler = async function(evt) {
  ...
  if (evt.target &&
    ...
    await cvox.ChromeVoxEventWatcher.setLastFocusedNode_(null);
  ...
  if (evt.target && evt.target != window) {
    ...
    if (parentControl) {
      ...
      await cvox.ChromeVoxEventWatcher.setLastFocusedNode_(target);
    }

    var queueMode = cvox.ChromeVoxEventWatcher.queueMode_();

    if (cvox.ChromeVoxEventWatcher.getInitialVisibility() ||
        await cvox.ChromeVoxEventWatcher.handleDialogFocus(target)) {
    ...
    if (evt.target.hasAttribute) {
      ...
      switch (inputType) {
        case 'time':
          await cvox.ChromeVoxEventWatcher.setUpTimeHandler_();
          return;
        case 'date':
        case 'month':
        case 'week':
          await cvox.ChromeVoxEventWatcher.setUpDateHandler_();
          return;
      }
    }
    cvox.ChromeVoxEventWatcher.setUpTextHandler();
 ...
 */
cvox.ChromeVoxEventWatcher.handleControlChanged = async function(control) {
  var newValue = await cvox.DomUtil.getControlValueAndStateString(control);
  ...
  if (control != cvox.ChromeVoxEventWatcher.lastFocusedNode &&
      (parentControl == null ||
       parentControl != cvox.ChromeVoxEventWatcher.lastFocusedNode)) {
    await cvox.ChromeVoxEventWatcher.setLastFocusedNode_(control);
  ...
  if ((parentControl &&
    ...
    await cvox.ChromeVox.navigationManager.updateSelToArbitraryNode(
        activeDescendant,
        true);
 ...
 */
cvox.ChromeVoxEventWatcher.handleDialogFocus = async function(target) {
  var dialog = target;
  var role = '';
  while (dialog) {
  ...
  if (cvox.ChromeVox.navigationManager.currentDialog && !dialog) {
    ...
    if (dialog) {
      ...
      if (role == 'alertdialog') {
        var dialogDescArray =
            await cvox.DescriptionUtil.getFullDescriptionsFromChildren(null, dialog);
        var descSpeaker = new cvox.NavigationSpeaker();
 ...
 * @private
 */
cvox.ChromeVoxEventWatcher.doProcessQueue_ = async function() {
  ...
  for (i = 0; evt = cvox.ChromeVoxEventWatcher.events_[i]; i++) {
    await cvox.ChromeVoxEventWatcher.handleEvent_(evt);
  }
  cvox.ChromeVoxEventWatcher.events_ = new Array();
  cvox.ChromeVoxEventWatcher.firstUnprocessedEventTime = -1;
  cvox.ChromeVoxEventWatcher.queueProcessingScheduled_ = false;
 ...
 */
cvox.ChromeVoxEventWatcher.handleEvent_ = async function(evt) {
  switch (evt.type) {
    case 'keydown':
    case 'input':
      cvox.ChromeVoxEventWatcher.setUpTextHandler();
    ...
    case 'click':
      ...
      break;
    case 'focus':
      await cvox.ChromeVoxEventWatcher.focusHandler(evt);
      break;
    case 'blur':
      cvox.ChromeVoxEventWatcher.setUpTextHandler();
      break;
 ...
 * @private
 */
cvox.ChromeVoxEventWatcher.setUpTimeHandler_ = async function() {
  var currentFocus = document.activeElement;
  if (currentFocus &&
      currentFocus.hasAttribute &&
      currentFocus.getAttribute('aria-hidden') == 'true' &&
  ...
  if (currentFocus.constructor == HTMLInputElement &&
    ...
    cvox.ChromeVoxEventWatcher.currentTimeHandler =
        ...
        await cvox.ChromeVoxHTMLTimeWidget.create(currentFocus, cvox.ChromeVox.tts);
    } else {
 ...
 * @private
 */
cvox.ChromeVoxEventWatcher.setUpDateHandler_ = async function() {
  var currentFocus = document.activeElement;
  if (currentFocus &&
      currentFocus.hasAttribute &&
      currentFocus.getAttribute('aria-hidden') == 'true' &&
  ...
  if (currentFocus.constructor == HTMLInputElement &&
    ...
    cvox.ChromeVoxEventWatcher.currentDateHandler =
        ...
        await cvox.ChromeVoxHTMLDateWidget.create(currentFocus, cvox.ChromeVox.tts);
    } else {
