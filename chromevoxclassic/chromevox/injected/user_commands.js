 ...
 * @private
 */
cvox.ChromeVoxUserCommands.handleTabAction_ = async function() {
  ...
  if (!(await cvox.ChromeVox.navigationManager.resolve(cvox.DomUtil.isFocusable))) {
...
cvox.ChromeVoxUserCommands.createCommand_ = function(cmd) {
  return goog.bind(async function(opt_kwargs) {
    var cmdStruct = cvox.ChromeVoxUserCommands.lookupCommand_(cmd, opt_kwargs);
    return await cvox.ChromeVoxUserCommands.dispatchCommand_(cmdStruct);
 ...
 * @private
 */
cvox.ChromeVoxUserCommands.dispatchCommand_ = async function(cmdStruct) {
  ...
  return await cvox.ChromeVoxUserCommands.doCommand_(cmdStruct);
};
 ...
 * @private
 */
cvox.ChromeVoxUserCommands.doCommand_ = async function(cmdStruct) {
  if (cvox.Widget.isActive()) {
    return true;
  }
  ...
  switch (cmd) {
    case 'handleTab':
    case 'handleTabPrev':
      ret = await cvox.ChromeVoxUserCommands.handleTabAction_();
      break;
    case 'forward':
    case 'backward':
      ret = !(await cvox.ChromeVox.navigationManager.navigate());
      break;
    case 'right':
    case 'left':
      await cvox.ChromeVox.navigationManager.subnavigate();
      break;
    case 'find':
      if (!cmdStruct.findNext) {
      ...
      switch (status) {
        ...
        case cvox.UserEventDetail.Status.FAILURE:
          ...
          break;
        default:
          found = await cvox.ChromeVox.navigationManager.findNext(
    ...
    case 'previousGranularity':
      await cvox.ChromeVox.navigationManager.makeLessGranular(true);
      prefixMsg = cvox.ChromeVox.navigationManager.getGranularityMsg();
      break;
    case 'nextGranularity':
      await cvox.ChromeVox.navigationManager.makeMoreGranular(true);
      prefixMsg = cvox.ChromeVox.navigationManager.getGranularityMsg();
      break;

    case 'previousCharacter':
      await cvox.ChromeVox.navigationManager.navigate(false,
    ...
    case 'nextRow':
    case 'nextCol':
      await cvox.ChromeVox.navigationManager.performAction('enterShifterSilently');
      await cvox.ChromeVox.navigationManager.performAction(cmd);
      break;
    ...
    case 'showNextUpdatePage':
      ...
      break;
    case 'fullyDescribe':
      var descs = await cvox.ChromeVox.navigationManager.getFullDescription();
  ...
  if (errorMsg != '') {
    ...
    if (cmdStruct.announce) {
      await cvox.ChromeVox.navigationManager.finishNavCommand(prefixMsg);
    }
  }
  if (!cmdStruct.allowEvents) {
 ...
 */
cvox.ChromeVoxUserCommands.handleChromeVoxUserEvent = async function(cvoxUserEvent) {
  var detail = new cvox.UserEventDetail(cvoxUserEvent.detail);
  if (detail.command) {
    await cvox.ChromeVoxUserCommands.doCommand_(
