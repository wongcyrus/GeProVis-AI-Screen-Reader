 ...
 * @private
 */
cvox.ChromeVox.recallInit_ = async function(reason) {
  ...
  cvox.ChromeVox.initTimer_ = window.setTimeout(
      await cvox.ChromeVox.initDocument,
 ...
 */
cvox.ChromeVox.initDocument = async function() {
  ...
  if (reinitReason) {
    await cvox.ChromeVox.recallInit_(reinitReason + ': ' +
  ...
  if (!cvox.ChromeVox.host.ttsLoaded()) {
    await cvox.ChromeVox.recallInit_('ChromeVox not starting; waiting for TTS. ' +
                               document.location.href + '.');
    return;
  }
  ...
  await cvox.InitGlobals.initGlobals();
  ...
  window.disableChromeVox = async function() {
    await cvox.ChromeVox.host.killChromeVox();
  };
};
 ...
 */
cvox.ChromeVox.reinit = async function() {
  await cvox.ChromeVox.host.reinit();
  await cvox.ChromeVox.initDocument();
};
...
if (!COMPILED) {
  ...
  // called in loader.js.
  // window.console.log("init document directly from init_document");
  await cvox.ChromeVox.initDocument();
}
})();
