 ...
 */
cvox.TableWalker.prototype.getDescription = async function(prevSel, sel) {
  var position = this.syncPosition_(sel);
  if (!position) {
    return [];
  }
  this.tt.goToCell(position);
  var descs = await cvox.DescriptionUtil.getCollectionDescription(prevSel, sel);
  if (descs.length == 0) {
 ...
 * @override
 */
cvox.TableWalker.prototype.getBraille = async function(prevSel, sel) {
  var ret = new cvox.NavBraille({});
  var position = this.syncPosition_(sel);
  if (position) {
    var text =
        await cvox.BrailleUtil.getTemplated(prevSel.start.node, sel.start.node);
 ...
 * @export
 */
cvox.TableWalker.prototype.announceHeaders = async function(sel) {
  cvox.ChromeVox.tts.speak(await this.getHeaderText_(sel),
 ...
 * @private
 */
cvox.TableWalker.prototype.getHeaderText_ = async function(sel) {
  ...
  return (
      await this.getRowHeaderText_(position) +
      ' ' +
      await this.getColHeaderText_(position));
};
 ...
 * @private
 */
cvox.TableWalker.prototype.getRowHeaderText_ = async function(position) {
  ...
  if (rowHeaders.length == 0) {
    ...
    rowHeaderText += cvox.DomUtil.collapseWhitespace(
        await cvox.DomUtil.getValue(firstCellInRow) + ' ' +
            await cvox.DomUtil.getName(firstCellInRow));
    return Msgs.getMsg('row_header') + rowHeaderText;
  }

  for (var i = 0; i < rowHeaders.length; ++i) {
    rowHeaderText += cvox.DomUtil.collapseWhitespace(
        await cvox.DomUtil.getValue(rowHeaders[i]) + ' ' +
            await cvox.DomUtil.getName(rowHeaders[i]));
  }
  if (rowHeaderText == '') {
 ...
 * @private
 */
cvox.TableWalker.prototype.getColHeaderText_ = async function(position) {
  // TODO(stoarca): OPTMZ Replace with join();
  var colHeaderText = '';
  ...
  if (colHeaders.length == 0) {
    ...
    colHeaderText += cvox.DomUtil.collapseWhitespace(
        await cvox.DomUtil.getValue(firstCellInCol) + ' ' +
        await cvox.DomUtil.getName(firstCellInCol));
    return Msgs.getMsg('column_header') + colHeaderText;
  }

  for (var i = 0; i < colHeaders.length; ++i) {
    colHeaderText += cvox.DomUtil.collapseWhitespace(
        await cvox.DomUtil.getValue(colHeaders[i]) + ' ' +
            await cvox.DomUtil.getName(colHeaders[i]));
  }
  if (colHeaderText == '') {
    return Msgs.getMsg('empty_row_header');
  }
