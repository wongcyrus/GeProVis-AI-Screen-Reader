 ...
 * @override
 */
cvox.LayoutLineWalker.prototype.next = async function(sel) {
  // Collapse selection to the directed end.
  var endSel = new cvox.CursorSelection(sel.end, sel.end, sel.isReversed());

  // Sync to the line.
  var sync = await this.subWalker_.sync(endSel);
  if (!sync) {
    return null;
  }

  // Compute the next selection.
  var start = await this.subWalker_.next(endSel);
  if (!start) {
    return null;
  }
  start.setReversed(sel.isReversed());
  return await this.extend_(start).setReversed(false);
};
 ...
 * @override
 */
cvox.LayoutLineWalker.prototype.sync = async function(sel) {
  var line = await this.subWalker_.sync(sel);
  if (!line) {
    return null;
  }

  // Extend to both line breaks (in each direction).
  var end = await this.extend_(line);
  var start = await this.extend_(line.setReversed(!line.isReversed()));
 ...
 * @override
 */
cvox.LayoutLineWalker.prototype.getDescription = async function(prevSel, sel) {
  var descriptions = [];
  var prev = prevSel;
  var absSel = sel.clone().setReversed(false);
  var cur = new cvox.CursorSelection(absSel.start, absSel.start);
  cur = await this.subWalker_.sync(cur);
  if (!cur) {
    return [];
  }
  ...
  if (absSel.start.node == absSel.end.node) {
    return await this.subWalker_.getDescription(prevSel, sel);
  }

  // Walk through and collect descriptions for each line.
  while (cur && !cur.end.equals(absSel.end)) {
    descriptions.push.apply(
        descriptions, await this.subWalker_.getDescription(prev, cur));
    prev = cur;
    cur = await this.subWalker_.next(cur);
  }
  if (cur) {
    descriptions.push.apply(
        descriptions, await this.subWalker_.getDescription(prev, cur));
 ...
 * @override
 */
cvox.LayoutLineWalker.prototype.getBraille = async function(prevSel, sel) {
  var braille = new cvox.NavBraille({});
  var absSel = await this.subWalker_.sync(sel.clone().setReversed(false));
  var layoutSel = (await this.sync(sel)).setReversed(false);
  if (!layoutSel || !absSel) {
    return braille;
  }
  ...
  while (cur && !cur.end.equals(layoutSel.end)) {
    await this.appendBraille_(prevSel, absSel, cur, braille);
    prevSel = cur;
    cur = await this.subWalker_.next(cur);
  }
  if (cur) {
 ...
 * @private
 */
cvox.LayoutLineWalker.prototype.extend_ = async function(start) {
  ...
  do {
    ...
    next = await this.subWalker_.next(end);
 ...
 * @private
 */
cvox.LayoutLineWalker.prototype.appendBraille_ = async function(
    prevSel, sel, cur, braille) {
  var item = (await this.subWalker_.getBraille(prevSel, cur)).text;
