...
ChromeVoxWalkerUnitTestBase.prototype = {
   ...
   */
  go: async function(sel, opt_cmdOrDest, desc) {
    ...
    var trueDesc = await this.walker.getDescription(sel, ret)[0];
...
ChromeVoxWalkerUnitTestBase.addCommonTests = function(testFixture) {
   ...
   */
  TEST_F(testFixture, 'testSyncInvariant', async function() {
    this.loadDoc(function() {/*!
      <div id="outer">
        <p id="a">a</p>
        <p id="b">b</p>
    ...
    */});
    var sel = cvox.CursorSelection.fromNode(
        $('outer').firstChild);
    var sync = await this.walker.sync(sel);
    var syncsync = await this.walker.sync(sync);
    assertEquals(true, sync.equals(syncsync));

    sel = cvox.CursorSelection.fromNode($('a'));
    sync = await this.walker.sync(sel);
    syncsync = await this.walker.sync(sync);
    assertEquals(true, sync.equals(syncsync));
