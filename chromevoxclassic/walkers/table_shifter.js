 ...
 * @override
 */
cvox.TableShifter.prototype.next = async function(sel) {
  var nextSel = await this.currentWalker_.next(sel);
  if (!nextSel) {
    // Bumped edge.
 ...
 * @override
 */
cvox.TableShifter.prototype.sync = async function(sel) {
 ...
 */
cvox.TableShifter.prototype.getDescription = async function(prevSel, sel) {
  var descs = await this.currentWalker_.getDescription(prevSel, sel);
  if (descs.length > 0) {
    if (this.bumpedEdge_) {
 ...
 * @override
 */
cvox.TableShifter.prototype.getBraille = async function(prevSel, sel) {
  return await this.currentWalker_.getBraille(prevSel, sel);
};
