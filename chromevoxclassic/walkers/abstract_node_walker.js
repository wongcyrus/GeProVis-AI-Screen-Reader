 ...
 * @override
 */
cvox.AbstractNodeWalker.prototype.next = async function(sel) {
 ...
 * @override
 */
cvox.AbstractNodeWalker.prototype.sync = async function(sel) {
  var ret = await this.privateSync_(sel);
  this.wasBegin_ = false;
  return ret;
};
 ...
 * @private
 */
cvox.AbstractNodeWalker.prototype.privateSync_ = async function(sel) {
  var r = sel.isReversed();
  ...
  if (!(await cvox.DomUtil.hasContent(node))) {
    n = await this.next(/** @type {!cvox.CursorSelection} */
