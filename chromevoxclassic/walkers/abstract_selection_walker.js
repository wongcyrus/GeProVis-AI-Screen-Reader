 ...
 * @override
 */
cvox.AbstractSelectionWalker.prototype.next = async function(sel) {
  ...
  if (ret == null) {
    ...
    return await this.objWalker_.next(sel);
  }
  var retSel = this.tc_.getCurrentCursorSelection().setReversed(r);
  var objSel = await this.objWalker_.next(sel);
 ...
 * @override
 */
cvox.AbstractSelectionWalker.prototype.sync = async function(sel) {
  var r = sel.isReversed();
  var newSel = null;
  ...
  newSel = await this.next(newSel.setReversed(false));

  // ObjectWalker wins when there's a discrepancy between it and
  // TraverseContent. The only exception is with an end cursor on a text node.
  // In all other cases, this makes sure we visit the same selections as
  // object walker.
  var objSel = await this.objWalker_.sync(sel);
  objSel = objSel ? objSel.setReversed(r) : null;
 ...
 * @override
 */
cvox.AbstractSelectionWalker.prototype.getDescription = async function(prevSel, sel) {
