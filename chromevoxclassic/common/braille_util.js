 ...
 */
cvox.BrailleUtil.getName = async function(node) {
  if (!node) {
    return '';
  }
  return (await cvox.DomUtil.getName(node)).trim();
};
 ...
 */
cvox.BrailleUtil.getValue = async function(node) {
  if (!node) {
  ...
  if (cvox.DomUtil.isInputTypeText(node)) {
    ...
    return new Spannable(await cvox.DomUtil.getValue(node), valueSpan);
  }
};
 ...
 */
cvox.BrailleUtil.getTemplated = async function(prev, node, opt_override) {
  ...
  var mapChar = async function(c) {
    switch (c) {
      case 'n':
        return opt_override.name || await cvox.BrailleUtil.getName(node);
      case 'r':
        return role;
      case 's':
        return state;
      ...
      case 'v':
        return opt_override.value || await cvox.BrailleUtil.getValue(node);
      default:
        return c;
    }
  };
  for (var i = 0; i < template.length; i++) {
    var component = await mapChar(template[i]);
