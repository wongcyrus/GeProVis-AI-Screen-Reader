...
cvox.ChromeVoxHTMLTimeWidget = function(timeElem, tts) {
  ...
  this.pos_ = 0;
  this.maxPos_ = 2;
  this.keyListener_ = async function(evt) {
    await self.eventHandler_(evt);
  };
  ...
  // await this.update_(true);  //move to the new create method
};

/**
 * create method to construct the widget
 */
cvox.ChromeVoxHTMLTimeWidget.create = async function(timeElem, tts) {
  timeWidget = new cvox.ChromeVoxHTMLTimeWidget(timeElem, tts);

  await timeWidget.update_(true);
  return timeWidget;
}
 ...
 * @private
 */
cvox.ChromeVoxHTMLTimeWidget.prototype.update_ = async function(shouldSpeakLabel) {
  ...
  if (shouldSpeakLabel) {
    changeMessage = await cvox.DomUtil.getName(this.timeElem_, true, true) + '\n';
  }
 ...
 * @private
 */
cvox.ChromeVoxHTMLTimeWidget.prototype.eventHandler_ = async function(evt) {
  ...
  await this.update_(shouldSpeakLabel);
};
