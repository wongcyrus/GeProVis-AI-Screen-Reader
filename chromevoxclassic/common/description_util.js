...
cvox.DescriptionUtil.getControlDescription =
    async function(control, opt_changedAncestors) {
  ...
  var description = await cvox.DescriptionUtil.getDescriptionFromAncestors(
      ancestors, true, cvox.VERBOSITY_VERBOSE);

  // Use heuristics if the control doesn't otherwise have a name.
  if (surroundingControl) {
    var name = await cvox.DomUtil.getName(surroundingControl);
    if (name.length == 0) {
      name = await cvox.DomUtil.getControlLabelHeuristics(surroundingControl);
      if (name.length > 0) {
        description.context = name + ' ' + description.context;
      }
    }
  } else {
    var name = await cvox.DomUtil.getName(control);
    if (name.length == 0) {
      name = await cvox.DomUtil.getControlLabelHeuristics(control);
      if (name.length > 0) {
        description.text = cvox.DomUtil.collapseWhitespace(name);
      }
    }
    var value = await cvox.DomUtil.getValue(control);
    if (value.length > 0) {
 ...
 */
cvox.DescriptionUtil.getDescriptionFromAncestors = async function(
  ...
  if (len > 0) {
    text = await cvox.DomUtil.getName(ancestorsArray[len - 1], recursive);

    userValue = await cvox.DomUtil.getValue(ancestorsArray[len - 1]);
  }
  for (var i = len - 1; i >= 0; i--) {
    var node = ancestorsArray[i];

    hint = await cvox.DomUtil.getHint(node);
    ...
    if (i < len - 1 && node.hasAttribute('role')) {
      var name = await cvox.DomUtil.getName(node, false);
      if (name) {
...
cvox.DescriptionUtil.getDescriptionFromNavigation =
    async function(prevNode, node, recursive, verbosity) {
  if (!prevNode || !node) {
    return [];
  }
  ...
  if (cvox.DescriptionUtil.COLLECTION_NODE_TYPE[node.tagName]) {
    return await cvox.DescriptionUtil.getCollectionDescription(
  ...
  var desc = await cvox.DescriptionUtil.getDescriptionFromAncestors(
  ...
  if (cvox.DescriptionUtil.shouldDescribeExit_(prevAncestors)) {
    var prevDesc = await cvox.DescriptionUtil.getDescriptionFromAncestors(
 ...
 */
cvox.DescriptionUtil.getCollectionDescription = async function(prevSel, sel) {
  var descriptions = await cvox.DescriptionUtil.getRawDescriptions_(prevSel, sel);
 ...
 * @private
 */
cvox.DescriptionUtil.getRawDescriptions_ = async function(prevSel, sel) {
  ...
  var node = (await cvox.DescriptionUtil.subWalker_.sync(sel)).start.node;
  ...
  while (cvox.DomUtil.isDescendantOfNode(node, sel.start.node)) {
    ...
    curSel = await cvox.DescriptionUtil.subWalker_.next(curSel);
    if (!curSel) {
      break;
    }
...
cvox.DescriptionUtil.getFullDescriptionsFromChildren =
  ...
  if (cvox.DomUtil.isLeafNode(node)) {
    ...
    desc = await cvox.DescriptionUtil.getDescriptionFromAncestors(
        ancestors, true, cvox.ChromeVox.verbosity);
  ...
  var newNode = await cvox.DescriptionUtil.subWalker_.sync(curSel).start.node;
  ...
  while (newNode && cvox.DomUtil.isDescendantOfNode(newNode, originalNode)) {
    descriptions = descriptions.concat(
        await cvox.DescriptionUtil.getFullDescriptionsFromChildren(
            prevnode, newNode));
    curSel = await cvox.DescriptionUtil.subWalker_.next(curSel);
    if (!curSel) {
      break;
