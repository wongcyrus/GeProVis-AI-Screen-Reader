 ...
 * @private
 */
cvox.DomUtil.getBaseLabel_ = async function(node, recursive, includeControls) {
  var label = '';
  if (node.hasAttribute) {
    if (node.hasAttribute('aria-labelledby')) {
      ...
      for (var i = 0; i < labelNodeIds.length; i++) {
        ...
        if (labelNode) {
          label += ' ' + await cvox.DomUtil.getName(
      ...
      label = await cvox.DomUtil.getImageTitle(node);
      ...
      for (var legend, i = 0; legend = legends[i]; i++) {
        label += ' ' + await cvox.DomUtil.getName(legend, true, includeControls);
      }
    }
    ...
    if (label.length == 0 && node && node.id) {
      ...
      if (labelFor) {
        label = await cvox.DomUtil.getName(labelFor, recursive, includeControls);
      }
    }
  }
 ...
 * @private
 */
cvox.DomUtil.getNearestAncestorLabel_ = async function(node) {
  var label = '';
  ...
  if (enclosingLabel && !enclosingLabel.hasAttribute('for')) {
    ...
    label = await cvox.DomUtil.getName(enclosingLabel, true, false);
  }
  return label;
};
 ...
 * @private
 */
cvox.DomUtil.getInputName_ = async function(node) {
  var label = '';
  if (node.type == 'image') {
    label = await cvox.DomUtil.getImageTitle(node);
 ...
 */
cvox.DomUtil.getName = async function(
  ...
  var ret =
      await cvox.DomUtil.getName_(node, recursive, includeControls, opt_allowHidden);
 ...
 * @private
 */
cvox.DomUtil.getName_ = async function(
    node, recursive, includeControls, opt_allowHidden) {
  ...
  var label = await cvox.DomUtil.getBaseLabel_(node, recursive, includeControls);

  if (label.length == 0 && cvox.DomUtil.isControl(node)) {
    label = await cvox.DomUtil.getNearestAncestorLabel_(node);
  }

  if (label.length == 0 && node.constructor == HTMLInputElement) {
    label = await cvox.DomUtil.getInputName_(node);
  }

  if (cvox.DomUtil.isInputTypeText(node) && node.hasAttribute('placeholder')) {
    var placeholder = node.getAttribute('placeholder');
    if (label.length > 0) {
      if (await cvox.DomUtil.getValue(node).length > 0) {
        return label;
      } else {
  ...
  if (cvox.DomUtil.hasChildrenBasedName_(node, opt_allowHidden)) {
    return await cvox.DomUtil.getNameFromChildren(
 ...
 */
cvox.DomUtil.getNameFromChildren = async function(
    node, includeControls, opt_allowHidden) {
  ...
  for (var i = 0; i < node.childNodes.length; i++) {
    ...
    if (opt_allowHidden || (isVisible && !cvox.AriaUtil.isHidden(child))) {
      delimiter = (prevChild.tagName == 'SPAN' ||
          ...
          '' : ' ';
      name += delimiter + await cvox.DomUtil.getName(child, true, includeControls);
    }
  }
 ...
 */
cvox.DomUtil.getControlLabelHeuristics = async function(node) {
  ...
  while (prevNode && (!(await cvox.DomUtil.hasContent(prevNode)) ||
  ...
  while (nextNode && (!(await cvox.DomUtil.hasContent(nextNode)) ||
  ...
  if (guessedLabelNode) {
    return cvox.DomUtil.collapseWhitespace(
        await cvox.DomUtil.getValue(guessedLabelNode) + ' ' +
        await cvox.DomUtil.getName(guessedLabelNode));
  }
 ...
 */
cvox.DomUtil.getValue = async function(node) {
  var activeDescendant = cvox.AriaUtil.getActiveDescendant(node);
  if (activeDescendant) {
    return cvox.DomUtil.collapseWhitespace(
        await cvox.DomUtil.getValue(activeDescendant) + ' ' +
        await cvox.DomUtil.getName(activeDescendant));
  }
  ...
  if (node.isContentEditable) {
    return await cvox.DomUtil.getNameFromChildren(node, true);
  }
...
cvox.DomUtil.getImageCaption = async function(url, base64) {
  ...
  } catch (e) { /*window.console.log(e);*/ return ""}
}
 ...
 */
cvox.DomUtil.hasContent = async function(node) {
  ...
  return /** @type {boolean} */ (cvox.Memoize.memoize(
      await cvox.DomUtil.computeHasContent_.bind(this), 'hasContent', node));
};
 ...
 * @private
 */
cvox.DomUtil.computeHasContent_ = async function(node) {
  ...
  var text = await cvox.DomUtil.getValue(node) + ' ' + await cvox.DomUtil.getName(node);
 ...
 */
cvox.DomUtil.getControlValueAndStateString = async function(control) {
  var parentControl = cvox.DomUtil.getSurroundingControl(control);
  if (parentControl) {
    return cvox.DomUtil.collapseWhitespace(
        await cvox.DomUtil.getValue(control) + ' ' +
        await cvox.DomUtil.getName(control) + ' ' +
 ...
 */
cvox.DomUtil.getFirstLeafNode = async function() {
  var node = document.body;
  while (node && node.firstChild) {
    node = node.firstChild;
  }
  while (node && !(await cvox.DomUtil.hasContent(node))) {
 ...
 */
cvox.DomUtil.getHint = async function(node) {
  var desc = '';
  if (node.hasAttribute) {
    if (node.hasAttribute('aria-describedby')) {
      ...
      for (var describedById, i = 0; describedById = describedByIds[i]; i++) {
        ...
        if (describedNode) {
          desc += ' ' + await cvox.DomUtil.getName(
