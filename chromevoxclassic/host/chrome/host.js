...
/** @override */
cvox.ChromeHost.prototype.init = async function() {
  ...
  var self = this;
  var listener = async function(message) {
    if (message['history']) {
    ...
    if (message['prefs']) {
      ...
      await self.activateOrDeactivateChromeVox(prefs['active'] == 'true');
      self.activateOrDeactivateStickyMode(prefs['sticky'] == 'true');
      if (!self.gotPrefsAtLeastOnce_) {
        await cvox.InitialSpeech.speak();
      }
      ...
      if (apiPrefsChanged) {
        var searchInit = prefs['siteSpecificEnhancements'] === 'true' ?
            ...
            undefined;
        await cvox.ApiImplementation.init(searchInit);
      }
  ...
  cvox.ExtensionBridge.addMessageListener(async function(msg, port) {
