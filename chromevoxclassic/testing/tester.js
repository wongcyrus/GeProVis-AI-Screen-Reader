 ...
 */
cvox.ChromeVoxTester.setUp = async function(doc) {

  // cvox.ChromeVox.navigationManager = new cvox.NavigationManager();
  // await cvox.ChromeVox.navigationManager.init(); //added to support image captioning
  ...
  await cvox.LiveRegions.init(new Date(0), cvox.QueueMode.QUEUE, false);

  await cvox.ChromeVoxEventWatcher.init(doc);
 ...
 */
cvox.ChromeVoxTester.setStrategy = async function(strategy) {
  await cvox.ChromeVox.navigationManager.ensureNotSubnavigating();
