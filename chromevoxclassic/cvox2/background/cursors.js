...
cursors.Cursor.prototype = {
   ...
   */
  move: async function(unit, movement, dir) {
    ...
    switch (unit) {
      case Unit.CHARACTER:
        ...
        if (newIndex < 0 || newIndex >= text.length) {
          newNode = await AutomationUtil.findNextNode(
      ...
      case Unit.DOM_NODE:
        switch (movement) {
          ...
          case Movement.DIRECTIONAL:
            ...
            newNode =
                await AutomationUtil.findNextNode(newNode, dir, pred) || originalNode;
            newIndex = cursors.NODE_INDEX;
            break;
        }
        break;
      case Unit.LINE:
        ...
        switch (movement) {
          case Movement.BOUND:
            newNode = await AutomationUtil.findNodeUntil(
