...
Background = function() {
  ...
  if (chrome.accessibilityPrivate && !chrome.accessibilityPrivate.setKeyboardListener)
...
Background.prototype = {
   ...
   */
  onGotCommand: async function(command) {
    ...
    if (pred) {
      ...
      if (bound) {
        var node = await AutomationUtil.findNextNode(
   ...
   */
  onBrailleKeyEvent: async function(evt, content) {
    if (this.mode === ChromeVoxMode.CLASSIC)
      return false;

    switch (evt.command) {
      case cvox.BrailleKeyCommand.PAN_LEFT:
        await this.onGotCommand('previousObject');
        break;
      case cvox.BrailleKeyCommand.PAN_RIGHT:
        await this.onGotCommand('nextObject');
        break;
      case cvox.BrailleKeyCommand.LINE_UP:
        await this.onGotCommand('previousLine');
        break;
      case cvox.BrailleKeyCommand.LINE_DOWN:
        await this.onGotCommand('nextLine');
        break;
      case cvox.BrailleKeyCommand.TOP:
        await this.onGotCommand('jumpToTop');
        break;
      case cvox.BrailleKeyCommand.BOTTOM:
        await this.onGotCommand('jumpToBottom');
        break;
      case cvox.BrailleKeyCommand.ROUTING:
        await this.brailleRoutingCommand_(
            content.text,
   ...
   * @private
   */
  onMessage_: async function(msg, port) {
    ...
    switch (target) {
      case 'next':
        if (action == 'getIsClassicEnabled') {
          ...
          await this.onGotCommand(msg['command']);
        } else if (action == 'flushNextUtterance') {
          await Output.flushNextSpeechUtterance();
        }
        break;
    }
  },
